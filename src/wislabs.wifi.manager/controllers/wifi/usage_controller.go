...
func SummaryDetailsFromTo(constrains dao.Constrains) [][]string {
	...
	for i := 1; i < len(CSVcontent) - 1; i++ {
		...
		CSVcontent[i][1] = dailyAccData[i].Username.String
		...
		CSVcontent[i][8] = strconv.FormatInt(dailyAccData[i].Outputoctets.Int64, 10)
		CSVcontent[i][9] = strconv.FormatInt(dailyAccData[i].Inputoctets.Int64,10)
		CSVcontent[i][10] = dailyAccData[i].Nasipaddress.String
		CSVcontent[i][11] = dailyAccData[i].Framedipaddress.String
		CSVcontent[i][12] = dailyAccData[i].Calledstationid.String
