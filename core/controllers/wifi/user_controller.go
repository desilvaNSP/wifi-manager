...
func GetUserCountOfDownloadsOver(constrains dao.Constrains, threshold int) int64{
	...
	if len(constrains.LocationId) >0 {
		smtOut, err := dbMap.Db.Prepare("SELECT count(*) FROM monthlyacct where date >= ? AND date < ? AND location = ? AND outputoctets >= ?")
		defer  smtOut.Close()
		...
		smtOut, err := dbMap.Db.Prepare("SELECT count(*) FROM monthlyacct where date >= ? AND date < ? AND outputoctets >= ?")
		defer  smtOut.Close()
...
func AddWiFiUser(user *dao.RadiusUser){
...
func GetAllWiFiUsers() []dao.RadiusUser{
	dbMap := utils.GetDBConnection("portal");
	defer dbMap.Db.Close()
	var users []dao.RadiusUser
...
func GetReturningUsers(from string, to string, location string) int64{
	...
	defer dbMap.Db.Close()
	var count int64
	count, err := dbMap.SelectInt("SELECT COUNT(*) FROM accounting where acctstarttime >= ? AND acctstarttime < ? AND location = ? AND visits > 1", from, to, location)
	checkErr(err, "Select failed")
	return count
}
